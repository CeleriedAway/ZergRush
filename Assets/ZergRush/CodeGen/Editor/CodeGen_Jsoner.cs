...
namespace ZergRush.CodeGen
{
    public static partial class CodeGen
    {
        ...
        static void GenerateJsonSerialization(Type type, string funcPrefix)
        {
            ...
            if (type.IsList() || type.IsArray)
            {
                ...
                if (elemType.IsDataNode())
                {
                    ...
                    ReadJsonValueStatement(sinkReader,
                        new DataInfo
                        {
                            type = elemType, baseAccess = $"self[self.{count} - 1]",
            ...
            {
                var sinkReader = MakeGenMethod(type, GenTaskFlags.JsonSerialization, funcPrefix + JsonReadFuncName,
                    Void, $"JsonTextReader {readerName}");
                var sinkWriter = MakeGenMethod(type, GenTaskFlags.JsonSerialization, funcPrefix + JsonWriteFuncName,
                    Void, $"JsonTextWriter {writerName}");
                ...
                WriteJsonValueStatement(sinkWriter,
                    new DataInfo
                        {type = keyType, baseAccess = $"item.Key", insideConfigStorage = type.IsConfigStorage()}, true);
                ...
                ReadJsonValueStatement(sinkReader,
                    new DataInfo
                    {
                        type = keyType, baseAccess = $"key", insideConfigStorage = type.IsConfigStorage(),
                        sureIsNull = true
                    }, true);
                ...
                ReadJsonValueStatement(sinkReader,
                    new DataInfo
                    {
                        type = valType, baseAccess = $"val", insideConfigStorage = type.IsConfigStorage(),
                        sureIsNull = true
                    }, true);
            ...
            else
            {
                ...
                var sinkReader = MakeGenMethod(type, GenTaskFlags.JsonSerialization, readerFuncName,
                    immutableMode ? type : Void,
                    $"{(immutableMode ? "this " : "")}JsonTextReader {readerName}{(!externalMode ? ", string __name" : "")}",
                    immutableMode);
                var sinkWriter = MakeGenMethod(type, GenTaskFlags.JsonSerialization,
                    funcPrefix + JsonWriteFuncName + (!externalMode ? "Fields" : ""), Void,
                    $"JsonTextWriter {writerName}");
                ...
                if (type.GetMembersForCodeGen(GenTaskFlags.JsonSerialization).Any())
                {
