...
namespace ZergRush.CodeGen
{
    public static partial class CodeGen
    {
        ...
        public static void RequestGen(Type t, Type requester, GenTaskFlags flags, bool allowGenericDeclRegister = false)
        {
            ...
            if (t.IsPrimitive || t.IsNullable() || t.IsEnum || t.IsGenericParameter || t == typeof(string) || t == typeof(byte[])) return;
...
//            }
        ...
        {
            if (t.IsGenericType && t.GenericTypeArguments.Any(par => par.IsGenericParameter)) return t.GetGenericTypeDefinition();
            else return t;
        }
        ...
        public static IEnumerable<DataInfo> GetMembersForCodeGen(this Type type, GenTaskFlags flagRestriction = GenTaskFlags.None, bool inheretedMembers = false, bool ignoreCheck = true)
        {
            ...
            if (ignoreCheck && inheretedMembers && membersForCodegenInheretedCache.TryGetValue(type, out var resultCached))
...
//                ignore = ignore | (field.HasAttribute<IgnoreFormatAttribute>() ? GenTaskFlags.All : GenTaskFlags.None);
                ...
                members.Add(new DataInfo{type = field.FieldType, baseAccess = field.Name, 
            ...
            foreach (var property in type.GetProperties(fieldFlags))
            {
                ...
                members.Add(new DataInfo{type = property.PropertyType, 
