...
public static partial class SerializationExtensions
{
    ...
    public static void ReadFromJson(this ZergRush.Alive.ConfigStorageList<ZergRush.Alive.GameLoadableConfigExample> self, JsonTextReader reader) 
    {
        if (reader.TokenType != JsonToken.StartArray) throw new JsonSerializationException("Bad Json Format");
        while (reader.Read())
        {
            if (reader.TokenType == JsonToken.EndArray) { break; }
            ZergRush.Alive.GameLoadableConfigExample val = default;
            val = new ZergRush.Alive.GameLoadableConfigExample();
            val.ReadFromJson(reader);
            GameConfigExample.Instance.RegisterConfig(val);
            self.Add(val);
        }
    }
    public static void WriteJson(this ZergRush.Alive.ConfigStorageList<ZergRush.Alive.GameLoadableConfigExample> self, JsonTextWriter writer) 
    {
        writer.WriteStartArray();
        for (int i = 0; i < self.Count; i++)
        {
    ...
    public static void CollectConfigs(this ZergRush.Alive.ConfigStorageList<ZergRush.Alive.GameLoadableConfigExample> self, ConfigRegister _collection) 
    {
        var size = self.Count;
        for (int i = 0; i < size; i++)
        {
    ...
    public static ulong CalculateHash(this ZergRush.Alive.ConfigStorageList<ZergRush.Alive.GameLoadableConfigExample> self) 
    {
        System.UInt64 hash = 345093625;
    ...
    public static void Serialize(this ZergRush.Alive.ConfigStorageList<ZergRush.Alive.GameLoadableConfigExample> self, BinaryWriter writer) 
    {
        writer.Write(self.Count);
        for (int i = 0; i < self.Count; i++)
        {
    ...
    public static void Deserialize(this ZergRush.Alive.ConfigStorageList<ZergRush.Alive.GameLoadableConfigExample> self, BinaryReader reader) 
    {
        var size = reader.ReadInt32();
        if(size > 1000) throw new ZergRushCorruptedOrInvalidDataLayout();
        self.Capacity = size;
