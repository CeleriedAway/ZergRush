...
public static class CodeGenTools
{
    ...
    public static string NameWithNamespace(this Type t)
    {
        ...
        if (string.IsNullOrWhiteSpace(@namespace) == false) return $"{@namespace}.{t.Name}";
        else return t.Name;
    }
    ...
    public static string RealName(this Type t, bool withNamespace = false)
    {
        ...
        if (t.IsGenericType && t.GetGenericTypeDefinition() == typeof (ValueTuple<>))
        {
            ...
            var tupleName = $"({tupleTypes.Select(a => a.RealName(withNamespace)).ToCodeGenListString()})";
            return tupleName;
        }
        ...
        if (t.IsGenericType)
        {
            ...
            name += $"<{t.GetGenericArguments().Select(a => a.RealName(!a.IsGenericParameter && withNamespace)).ToCodeGenListString()}>";
        }
