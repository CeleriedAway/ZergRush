...
public unsafe static class CodeGenImplTools
{
    ...
    {
        print($"{path.Reverse().ToCodeGenListString("/")}/{name} is different, self: {self, 18:0.000000000} other: {other, 18:0.000000000}");
    }

    public static void LogCompError<T>(Stack<string> path, string name, Action<string> print, T other, T self)
    {
        print($"{path.Reverse().ToCodeGenListString("/")}/{name} is different, self: {self} other: {other}");
    }
    ...
    public static bool CompareNullable<T>(Stack<string> path, string name, Action<string> printer, T val, T val2)
    {
        ...
        printer($"{path.Reverse().ToCodeGenListString("/")}/{name} is different, self: {pr(val)} other: {pr(val2)}");
        return false;
    }
    ...
    public static bool CompareClassId<T>(Stack<string> path, string name, Action<string> printer, T val, T val2) where T : IPolymorphable
    {
        if (val.GetClassId() != val2.GetClassId())
        {
            ...
            printer($"{path.Reverse().ToCodeGenListString("/")}/{name} class id do not mach, self: {pr(val)} other: {pr(val2)}");
            return false;
        }
    ...
    public static bool CompareRefs<T>(Stack<string> path, string name, Action<string> printer, T val, T val2)
    {
        ...
        {
            printer($"{path.Reverse().ToCodeGenListString("/")}/{name} class refs do not match");
            return false;
        }
