...
namespace ZergRush.CodeGen
{
	public static partial class CodeGen
	{
        ...
        public static bool IsList(this Type t)
        {
	        ...
	        return t.IsGenericType && typeof(IList<>).EnrichGeneric(t.FirstGenericArg()).IsAssignableFrom(t);
        }
...
//                name += $"_{t.GetGenericArguments().Select(a => a.RealName()).ToCodeGenListString("_")}";
//                return name;
//            }
//            return t.Name;
        }
		...
		public static string GenericParametersSuffix(this Type t)
		{
			...
			return $"<{unknownParameters.ToCodeGenListString()}>";
		}
		...
		public static string GenericParametersConstraints(this Type t)
		{
			...
			return string.Join(Environment.NewLine, t.UnknownGenericArguments()
				.Select(p =>
				{
					...
					string printedConstr = constraints.Select(c => c.RealName(true)).Distinct().ToCodeGenListString();
        ...
        public static string UniqueName(this Type t, bool withNamespace = true)
        {
			...
			{
				name = name.Substring(0, name.Length - 2);
				name += $"_{t.GetGenericArguments().Select(a => a.UniqueName(!a.IsGenericParameter && withNamespace)).ToCodeGenListString("_")}";
			}
			return name;
        }
