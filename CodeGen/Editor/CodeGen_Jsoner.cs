        ...
        static void GenerateJsonSerialization(Type type, string funcPrefix)
        {
            ...
            if (type.IsList() || type.IsArray)
            {
                ...
                if (elemType.IsDataNode())
                {
                    ...
                    ReadJsonValueStatement(sinkReader, 
                        new DataInfo {type = elemType, baseAccess = $"self[self.{count} - 1]", insideConfigStorage = type.IsConfigStorage(),
                            sureIsNull = true}, false);
                }
                else
                {
                    ReadJsonValueStatement(sinkReader, new DataInfo{type = info.type, baseAccess = $"val", 
            ...
            else if (type.IsDictionary())
            {
                ...
                sinkReader.openBrace();
                    ...
                    ReadJsonValueStatement(sinkReader, new DataInfo{type = keyType, baseAccess = $"key", insideConfigStorage = type.IsConfigStorage(), sureIsNull = true}, true);
                    sinkReader.content($"reader.Read();"); // val prop name
                    sinkReader.content($"reader.Read();"); // val content
                    ReadJsonValueStatement(sinkReader, new DataInfo{type = valType, baseAccess = $"val", insideConfigStorage = type.IsConfigStorage(), sureIsNull = true}, true);
